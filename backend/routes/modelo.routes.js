...
router.get("/:id", async (req, res) => {
  try {
    ...
    console.error(`Error al obtener modelo con ID ${req.params.id}:`, error)
    res.status(500).json({
      success: false,
      message: `Error al obtener modelo con ID ${req.params.id}`,
      error: error.message,
    })
  }
})
...
router.put("/:id", async (req, res) => {
  try {
    ...
    console.error(`Error al actualizar modelo con ID ${req.params.id}:`, error)
    res.status(500).json({
      success: false,
      message: `Error al actualizar modelo con ID ${req.params.id}`,
      error: error.message,
    })
  }
})
...
router.delete("/:id", async (req, res) => {
  try {
    ...
    console.error(`Error al eliminar modelo con ID ${req.params.id}:`, error)
    res.status(500).json({
      success: false,
      message: `Error al eliminar modelo con ID ${req.params.id}`,
      error: error.message,
    })
  }
})
